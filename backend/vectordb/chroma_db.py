...
class ChromaDBManager:
    ...
    def add_pdf(self, pdf_path, document_id):
        ...
        tfidf_matrix = self.tfidf_vectorizer.fit_transform(pdf_chunks)
        tfidf_keywords = [" ".join(np.array(self.tfidf_vectorizer.get_feature_names_out())[tfidf_matrix[i].toarray().argsort()[0][-5:]]) for i in range(len(pdf_chunks))]
    ...
    def get_documents(self, document_id):
        ...
        try:
            ...
            print(f"⚠️ Error retrieving documents for {document_id}: {e}")
